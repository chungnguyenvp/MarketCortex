...
from src.api.routes import analysis, history, backtest
from src.infrastructure.database.migrations import run_migrations

# Run database migrations on startup
run_migrations()

logger = logging.getLogger("marketcortex.api")

app = FastAPI(
    title="MarketCortex Multi-Agent System",
...
async def serve_index():
    ...
    return {"message": "MarketCortex API Backend is running. Add static/index.html to view web UI."}
...
async def health_check():
    ...
    return HealthCheckResponse(
        status="healthy",
        service="MarketCortex Multi-Agent API Engine",
        version="2.0.0"
    )
