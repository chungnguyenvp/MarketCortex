...
def synthesis_node(state: AgentState) -> Dict[str, Any]:
    ...
    score_result = calculate_recommendation_score(
        state.get("raw_financials", {}),
        state.get("technical_signals", {}),
        state.get("scraped_news", []),
    )
    ...
    try:
        ...
        prompt += (
            "\n\nQUANTITATIVE GUARDRAIL (computed from raw data): "
            f"score={score_result['score']:.2f}/100, "
            f"baseline recommendation={score_result['recommendation']}. "
            "Do not contradict this baseline without explicitly explaining the evidence."
        )
        ...
        return {
            ...
            "recommendation_breakdown": score_result,
        }
