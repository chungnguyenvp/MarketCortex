/**
 * MarketCortex Vanilla JavaScript Async REST Client
...
function initApp() {
    ...
    const scoreCoverage = document.getElementById("scoreCoverage");
    ...
    function renderResults(data) {
        ...
        const breakdown = data.recommendation_breakdown || {};
        if (recommendationScore) {
            recommendationScore.textContent = data.recommendation_score == null ? "--/100" : `${Number(data.recommendation_score).toFixed(1)}/100`;
        }
        if (scoreCoverage) {
            const coverage = breakdown.data_coverage == null ? "--" : `${Math.round(Number(breakdown.data_coverage) * 100)}%`;
            scoreCoverage.textContent = currentUiLang === "vi" ? `Độ phủ dữ liệu: ${coverage}` : `Data coverage: ${coverage}`;
        }
